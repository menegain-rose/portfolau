<script setup>
  ...
  let request = 'https://data.culture.gouv.fr/api/records/1.0/search/'
      ...
      +dep
      +'&facet=multiplexe'
      +'&refine.region_administrative=BOURGOGNE+%2F+FRANCHE-COMTE';
  ...
  .then(response => {
      ...
      listeCinema.value = response.records;
      // On vérifie dans la console l'obtention des résultats
      console.log("Liste des cinema", listeCinema);
